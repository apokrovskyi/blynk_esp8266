...
#include <Blynk/BlynkDebug.h>
...
#elif defined(PLATFORMIO) && defined(ESP8266)
    ...
    #define DELAY_CHUNK(x) ((x) < 65000 ? (x) : 65000)

    void BlynkDelay(millis_time_t ms)
    {
        millis_time_t timeout = ms*1000;

        while (timeout>0) {
            os_delay_us(DELAY_CHUNK(timeout));
            timeout-=DELAY_CHUNK(timeout);
        }
    }
